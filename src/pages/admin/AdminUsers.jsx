...
export default function AdminUsers() {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (!editForm.is_admin && selectedApis.length > 0) {
        await instance.put(`/api/admin/users/${editUser.id}/permissions`, {
          api_engine_ids: selectedApis,
        });
      }
  ...
  const confirmRevoke = async (data) => {
    setSaving(true);
    try {
      await instance.post(`/api/admin/users/${data.userId}/sync-permissions`, {
        action: "revoke",
      });
      toast.success("User updated.");
      ModalService.hide();
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
            ...
            {loading ? (
              ...
              users.map((u) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border ${MODE_BADGE[u.app_mode] ?? "bg-white/5 text-gray-400 border-white/10"}`}
                    >
                      {u.app_mode}
                    </span>
                  </td>
                      ...
                      <button
                        onClick={() => {
                          if (apiEngines.length === 0) {
                            instance
                              .get("/api/admin/api-engines")
                              .then((r) => setApiEngines(r.data));
                          }
                          setViewApiUser(u);
                        }}
              ...
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="min-w-0">
                  <div className="font-medium text-white text-sm flex items-center gap-2 flex-wrap">
                    {u.name}
                    {u.is_admin && (
                      <span className="text-xs px-1.5 py-0.5 rounded-full border bg-blue-500/20 text-blue-300 border-blue-500/30">
                        Admin
                      </span>
                    )}
                  </div>
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full border shrink-0 ${MODE_BADGE[u.app_mode] ?? "bg-white/5 text-gray-400 border-white/10"}`}
                >
                  {u.app_mode}
                </span>
              </div>
          ...
          <div className="bg-gray-900 border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md max-h-[92vh] overflow-y-auto">
            ...
            <div className="px-5 sm:px-6 py-4 sm:py-5">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-gray-400 text-xs uppercase tracking-widest mb-1.5 block">
                    Name
                  </label>
                  <input
                ...
                <div>
                  ...
                  <select
                    value={createForm.cms_role}
                    onChange={(e) =>
                      setCreateForm({
                        ...createForm,
                        cms_role: e.target.value,
                      })
                    }
                    className="w-full bg-gray-800 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-lime-500/50"
                  >
      ...
      {editUser && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-gray-900 border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md max-h-[92vh] overflow-y-auto custom-scrollbar">
            <div className="sm:hidden flex justify-center pt-3 pb-1">
              <div className="w-10 h-1 bg-white/20 rounded-full" />
            </div>
            <div className="px-5 sm:px-6 py-4 sm:py-5">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, app_mode: e.target.value })
                    }
                    className="w-full bg-gray-800 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-lime-500/50"
                  >
                    <option value="trial">Trial</option>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, cms_role: e.target.value })
                    }
                    disabled={
                      editUser.is_admin && editForm.cms_role === "supervisor"
                    }
                    className="w-full bg-gray-800 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-lime-500/50 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <option value="auditor">Auditor</option>
                    <option value="investigator">Investigator</option>
                    <option value="supervisor">Supervisor</option>
                  ...
                  {editUser.is_admin && editForm.cms_role === "supervisor" && (
                    <p className="text-xs text-gray-500 mt-1">
                      Supervisor role cannot be changed for admin users.
                    </p>
                  )}
                </div>
                    ...
                    <div className="mt-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {apiEngines.length > 0 && (
                          ...
                          >
                            {selectedApis.length === apiEngines.length
                              ? "Clear all"
                              : "Select all"}
                          </button>
                        )}
                        ...
                        {Object.entries(
                          apiEngines.reduce((acc, api) => {
                            const normalizedCategory =
                              api.category?.trim().toLowerCase() || "other";
              ...
              {viewApiUser.has_all_apis ? (
                ...
                <div className="space-y-3 mb-4">
                  {Object.entries(
                    apiEngines
                      .filter((api) =>
                        viewApiUser.api_engine_ids?.includes(api.id),
                      )
                      .reduce((acc, api) => {
                    ...
                    <div
                      key={category}
                      className="bg-gray-800/30 rounded-lg p-2.5 border border-white/5"
                    >
                      <div className="flex items-center gap-1.5 mb-1.5">
                        <div className="w-1 h-1 rounded-full bg-purple-400"></div>
                        <span className="text-purple-400 text-[10px] font-medium uppercase tracking-wider">
                          {category}
                        </span>
                      </div>
